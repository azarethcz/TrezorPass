    ...
    while (bezi) {
        ...
        switch (volba) {
            case 1: {
                ...
                if (trezor.empty()) {
                    TerminalUI::vytiskniRadekRamecku(TerminalUI::YELLOW + "Trezor je prázdný nebo došlo k chybě dešifrování." + TerminalUI::RESET);
                } else {
                    for (const auto& p : trezor) {
            ...
            case 3: {
                ...
                ulozTrezor(trezor);
                std::cout << TerminalUI::BRIGHT_GREEN << "\n[✓] Položka byla úspěšně zašifrována a uložena!\n" << TerminalUI::RESET;
                break;
            }
            case 4: {
                TerminalUI::smažVse();
